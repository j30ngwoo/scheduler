...
public class ScheduleController {
    ...
    @PostMapping
    public ApiResponse<Schedule> createSchedule(@RequestBody @Valid ScheduleCreateRequest request, @CurrentUser User currentUser) {
        Schedule created = scheduleService.createSchedule(request, currentUser);
        return ApiResponse.success(created);
    }

    @GetMapping
    public ApiResponse<List<Schedule>> getMySchedules(@CurrentUser User currentUser) {
        return ApiResponse.success(scheduleService.getSchedulesByUser(currentUser));
    }

    @GetMapping("/{code}")
    public ApiResponse<Schedule> getScheduleByCode(@PathVariable String code) {
